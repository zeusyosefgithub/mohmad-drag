'use client';
import { Button, Modal, Input, ModalBody, ModalContent, ModalFooter, ModalHeader, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Divider, AutocompleteItem, Autocomplete, Spinner, Switch, RadioGroup, Radio } from "@nextui-org/react";
...
import { useGetDataByCondition, useGetDataByConditionTwo, useGetDataByConditionWithoutUseEffect, useGetDataByLimit } from "../FireBase/getDataByCondition";
import { FaArrowDown, FaArrowUp, FaTrailer, FaTrash } from "react-icons/fa";
...
export default function ModalHtsgatHotsaa({ disable, show, hotsaa, aemTshlom, aobdem, sbkemA }) {
    ...
    const aglotDYetsor = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'D', 'sogAska', '==', 'ייצור');
    const aglotEYetsor = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'E', 'sogAska', '==', 'ייצור');

    const aglotDVav = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'D', 'sogAska', '==', 'הרכבת וו');
    const aglotEVav = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'E', 'sogAska', '==', 'הרכבת וו');

    const aglotDTekon = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'D', 'sogAska', '==', 'תיקון');
    const aglotETekon = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'E', 'sogAska', '==', 'תיקון');

    const aglotDTest = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'D', 'sogAska', '==', 'טסט');
    const aglotETest = useGetDataByConditionTwo('tfaol', 'sogBaola', '==', 'E', 'sogAska', '==', 'טסט');
    ...
    const handleSubmit = () => {
        setLoading(true);
        if (hotsaa.hotsaa === 'קניות מוצרים') {
            handleSubmitKneatMotsarem();
        }
        else if (hotsaa.hotsaa === 'הוצאות שכר') {
            handleSubmitHotsaotSkhar();
        }
        else if (hotsaa.hotsaa === 'הוצאות שוטפות') {
            handleSubmitShoteft();
        }
        else if (hotsaa.hotsaa === 'מסים') {
        ...
        else if (hotsaa.hotsaa === 'הוצאות אחרות') {
    ...
    const [sogHkhnsotYetsorAgla, setSogHkhnsotYetsorAgla] = useState('1');
    ...
    return (
        <Modal placement="center" className="test-fontt max-w-[1200px]" backdrop={"blur"} isOpen={show} onClose={ResetAll}>
            <ModalContent>
                <>
                    <ModalHeader className="shadow-2xl flex justify-center">{hotsaa?.baola} {hotsaa?.hotsaa}</ModalHeader>
                        ...
                        <div dir="rtl" className="p-5 w-full">
                            {
                                ...
                                <>
                                    ...
                                    {
                                        hotsaa?.hotsaa === 'שכירות' &&
                                        <div>

                                        </div>
                                    }
                                    {
                                        hotsaa?.hotsaa === 'הוצאות אחרות' &&
                                        <div>
                                            <div />
                                            <div className='overflow-auto h-[300px] flex justify-center w-full max-w-[900px] min-w-[400px]'>
                                                <form className='w-full'>
                            ...
                            }
                            {
                                ...
                                <>
                                    {
                                        hotsaa?.hotsaa === 'ייצור עגלות' &&
                                        <>
                                            <div>
                                                <RadioGroup
                                                    value={sogHkhnsotYetsorAgla}
                                                    onValueChange={(e) => setSogHkhnsotYetsorAgla(e)}>
                                                    <div className="flex items-center">
                                                        <Radio value="1"><div className="mr-1">מכירה</div></Radio>
                                                        <Radio value="2"><div className="mr-1">החזרה</div></Radio>
                                                    </div>
                                                </RadioGroup>
                                            </div>
                                            ...
                                            <div dir="ltr">
                                                {
                                                    sogHkhnsotYetsorAgla === '1' &&
                                                    <div className="overflow-x-auto h-[300px] w-full">
                                                        <table className="w-full table-auto border-collapse">
                                                            <thead>
                                                                <tr className="bg-gray-100 dark:bg-gray-800 sticky top-0 z-10">
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black">זמן עבר</th>
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black">תחילת תהליך</th>
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black">תאריך תחילה</th>
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black">סוג עסקה</th>
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black">מצב עסקה</th>
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black">מספר מכירה</th>
                                                                    <th className="px-4 py-2 text-center text-[13px] font-extrabold text-black w-20"></th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                {
                                                                    aglotDYetsor.map((agla, index) => {
                                                                        return <tr key={index} className="border-b border-gray-200 dark:border-gray-700">
                    ...
                    <ModalFooter>
                        <div className="w-full flex items-center justify-between">
                            ...
                            <div className="w-full flex items-center justify-end">
                                <Button size="lg" className="mr-5" variant="flat" color='warning' onClick={ResetAll}>
                                    סגור
                                </Button>
                                <Button isLoading={loading} onClick={handleSubmit} size="lg" variant="flat" color='success'>
                                    אישור
                                </Button>
                            </div>
                        </div>
